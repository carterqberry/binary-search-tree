...
private:
    ...
    bool level(Node* root, int lvl, stringstream& out) const;  //outputs nodes at a given level
...
template<typename T>
bool BST<T>::level(Node* root, int lvl, stringstream& out) const{  //outputs nodes at a given level
    if(root == NULL){
        return false;
    }
    if(lvl == 1){
    ...
    bool left = level(root->m_left, lvl - 1, out);
    bool right = level(root->m_right, lvl - 1, out);
    if((lvl == 2) && root->m_left && !root->m_right){
        out << " _";
    }
    return left || right;
...
string BST<T>::toString() const{  //return BST as a string in level order traversal
    ...
    if(m_root != NULL){  //if root isnt empty
        ...
        while(level(m_root, lvl, out));  //outputs the given level
    }
    return out.str();
}
